...
function onEditUser() {
  ...
  updateUserInfo(selectID.value, selectName.value, selectPremissopn.value, selectMail.value)
    .then((res) => {
        console.log(res)
        msg.value = '修改成功'
        updateData()
  ...
  <div class="tw-h-full tw-overflow-y-auto">
    ...
    <tbody>
      <tr v-for="user in users" :key="user.id">
        ...
        <td>{{ user.mail }}</td>
        <td>
          <v-btn @click="openUpdateDialog(user.id, user.name, user.mail, user.permission)"
            >修改</v-btn
          >
