...
df = pandas.read_csv('google_transit/shapes.csv')
...
data = {}
for shape_id, points in tqdm(lines_points.items()):
    features = []
    features.append(
        Feature(
            geometry = LineString(points),
            properties = {
                'line': routes[shape_id.split('.')[0]],
                'shape-id': shape_id,
                'color': colors[routes[shape_id.split('.')[0]]]
            }
        )
    )
    collection = FeatureCollection(features)
    data[shape_id] = collection

for shape_id, collection in data.items():
    with open(f"{shape_id}.geojson", "w") as f:
